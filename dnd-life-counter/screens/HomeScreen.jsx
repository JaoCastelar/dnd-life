...
export default function HomeScreen({ navigation }) {
  ...
  const renderItem = ({ item: personagem }) => (
    ...
    >
      ...
      <Text style={{ color: 'white' }}>Nível {personagem.nivel}</Text>
      <Text style={{ color: 'white' }}>Classe: {personagem.classes.join(', ')}</Text>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#000' }}>
      <View style={{ flex: 1 }}>
        <FlatList
          contentContainerStyle={{ padding: 20, paddingBottom: 100 }} // deixa espaço para o menu
          data={data}
